...
CCQGLView * CCQGLView::Create(float w, float h )
{
    ...
    s_pMainWindow->_window->setMousePressFunc(&cocos2d::mousePress);
    s_pMainWindow->_window->setMouseReleaseFunc(&cocos2d::mouseRelease);
	s_pMainWindow->_window->setWheelFunc(&cocos2d::wheel);

    s_pMainWindow->_window->setWindowFlags(s_pMainWindow->_window->windowFlags() & ~Qt::WindowMaximizeButtonHint);
    s_pMainWindow->_window->setBaseSize(width, height);
...
CCQGLView* CCQGLView::getInstance()
{
    ...
    {
        s_pMainWindow = CCQGLView::Create(960,640);
    }
    return s_pMainWindow;
}
